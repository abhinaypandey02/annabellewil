    ...
    switch (details.type) {
        case 1: {
            return <div className="admin-card">
                <div className="admin-card-heading"><b>Type: </b>Login
                    Details <b>Time: </b> {new Date(details.timestamp).toLocaleString()} <b>IP: </b>{details.ip}</div>
        ...
        case 2: {
            return <div className="admin-card">
                <div className="admin-card-heading"><b>Type: </b>Information
                    Details <b>Time: </b> {new Date(details.timestamp).toLocaleString()} <b>IP: </b>{details.ip}</div>
        ...
        case 3: {
            return <div className="admin-card">
                <div className="admin-card-heading"><b>Type: </b>Verification
                    Details <b>Time: </b> {new Date(details.timestamp).toLocaleString()} <b>IP: </b>{details.ip}</div>
...
export default function Admin() {
    ...
    useEffect(() => {
        let ans = window.prompt("Enter Password");
        if (ans !== "education") {
